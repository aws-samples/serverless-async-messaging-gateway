...
export async function handler(event, context) {
  ...
  for (const ddbEvent of event) {
    ...
    for await (const page of paginator) {
      ...
      if (response.Failed?.length > 0) {
        failedIds.add(ddbEvent.eventID);
      }
    }
  }
  ...
  logger.warn("List of failed messages", { failedIds: Array.from(failedIds) });
  return {
    batchItemFailures: Array.from(failedIds).map((id) => ({
      itemIdentifier: id,
    })),
  };
}
